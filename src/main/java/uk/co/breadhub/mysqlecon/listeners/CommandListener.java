        ...
        switch (args.length) {
            ...
            case 1: {
                if (args[0].toLowerCase().equals("balance")) {
                    ...
                    Main.getDatabase().userBalance(Bukkit.getOfflinePlayer(sender.getName()).getUniqueId());
                } else {
                    sendHelp(sender);
                }
                break;
                ...
                switch (args[0]) {
                    ...
                    case "deposit": {
                        if (Integer.parseInt(args[1]) >= 0.0001) {
                            if (Main.getDatabase().userBalance(Bukkit.getOfflinePlayer(args[1]).getUniqueId()) >= 0.0001) {
                    ...
                    switch (args[0]) {
                        case "deposit": {
                            if (Integer.parseInt(args[2]) >= 0.0001) {
                                Main.getDatabase().adminDeposit(Bukkit.getOfflinePlayer(sender.getName()).getUniqueId(), Integer.parseInt(args[2]));
                        ...
                        case "withdraw": {
                            if (Integer.parseInt(args[2]) >= 0.0001) {
                                ...
                                Main.getDatabase().adminWithdraw(Bukkit.getOfflinePlayer(sender.getName()).getUniqueId(), Integer.parseInt(args[2]));
                            } else {
                                sender.sendMessage(StringUtils.replaceColors("&4You cannot Withdraw Minus numbers or 0"));
                            }
                            break;
